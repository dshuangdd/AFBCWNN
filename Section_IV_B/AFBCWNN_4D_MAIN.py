...
# This implementation includes optional disturbance testing and memory optimization
# Key features: limited autodiff cache, RSS memory monitoring, adaptive dwell time
...
class AutomaticTrajectoryProcessor:
    ...
    def compute_derivatives(self, t: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # Cache size limitation with FIFO strategy
...
class HybridWaveletControl(nn.Module):
    def __init__(self, config, device):
        ...
        # IMPORTANT: Two ADAM optimizer implementations available
        # 1. High-precision version (default, slower but accurate)
        # 2. In-place version (commented out, faster but lower precision)
        # Users can switch between them by commenting/uncommenting the respective methods
        self.adam_m = []
        self.adam_v = []
        self.adam_t = 0
    ...
    def _compute_wavelet_terms_fast(self, x_est):
        ...
        if need_recompute:
            ...
            # Compute wavelets for each layer
            for layer_idx in range(len(self.weights)):
                if layer_idx == 0:
                    # Base layer: compute both phi and psi
                    j = self.j
    ...
    def _wavelet_function_batch(self, x, centers, j, is_base_layer):
        """Batch computation of wavelet functions (memory-optimized)"""
        scale_factor = self.power_cache.get(j, self.inv_scaling ** j)
        
        # For very large batches, compute in chunks to reduce memory peaks
        n_centers = len(centers)
        max_batch_size = 5000
    ...
    def _add_new_layer(self, parent_layer):
        ...
        # For mu=1, new layer's active_stage should be max_stage-1 (fully activated)
        if self.mu == 1:
            self.active_stages.append(self.max_stage - 1)
        else:
    ...
    def _check_dwell_time(self, t):
        ...
        if current_time >= self.current_dwell_start + self.dwell_time - 1e-6:
            if self.dwell_losses:
                ...
                # Save complete dwell period statistics
    ...
    def _collect_weight_history(self):
        """Collect weight history for variance analysis"""
    ...
    def _execute_pruning(self):
        ...
        if variance_info:
            ...
            if n_samples_for_variance:
                print(f"  Based on last {n_samples_for_variance} samples (last dwell time: ΔT={self.dwell_time/2:.2f})")
            else:
    ...
    # ========== ALTERNATIVE ADAM OPTIMIZER IMPLEMENTATIONS ==========
    # Users can choose between two versions by commenting/uncommenting:
    # 
    # Version 1 (COMMENTED OUT): In-place operations - Very fast but lower precision
    # Version 2 (DEFAULT): High precision - Slightly slower but better accuracy
    # 
    # To use the fast version: Comment out the method below and uncomment the alternative
    
    """
    # ALTERNATIVE 1: Fast in-place ADAM update (lower precision)
    def _adam_update_high_precision(self, layer_idx, delta, update_mask, beta1, beta2, eps, 
        ...
        if layer_idx == 0:
            if update_mask.any():
                # Use in-place operations for speed
    ...
    # DEFAULT: High-precision ADAM update (recommended)
    def _adam_update_high_precision(self, layer_idx, delta, update_mask, beta1, beta2, eps,
                                   bias_correction2, lr_adjusted, active_idx=None):
        """High-precision ADAM update with explicit computation"""
        w = self.weights[layer_idx]
        m = self.adam_m[layer_idx]
        v = self.adam_v[layer_idx]
    ...
    def _periodic_logging(self, t, deta):
        ...
        if current_time - self.last_print_time >= 0.05:
            # Count active parameters (excluding removed and frozen)
            param_count = 0
    ...
    def calculate_window_statistics(self, df):
        ...
        Prioritize using complete dwell_statistics if available
        """
        window_stats = []
        
        # Use complete dwell statistics if available
        if hasattr(self, 'dwell_statistics') and self.dwell_statistics:
            for i, dwell_stat in enumerate(self.dwell_statistics):
                # Get training_records data during this dwell period
    ...
    def save_to_excel(self, filename=None):
        ...
        if filename is None:
            ...
            filename = (f"training_report_AFBCWNN_{self.system_order}D_"
                       f"mu{mu_str}_dt{self.config['base_interval']}_"
                       f"lambda{self.lambda_c}_beta{self.beta}"
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            config_data = {
                'Parameter': ['system_order', 'lambda_c', 'beta', 'j_init', 'mu',
                            ...
                            'disturbance_enabled', 'disturbance_start', 'disturbance_end'],
                'Value': [self.system_order, self.lambda_c, self.beta,
                        ...
                        self.disturbance_enabled, 
                        self.disturbance_start_time if self.disturbance_enabled else 'N/A',
                        self.disturbance_end_time if self.disturbance_enabled else 'N/A']
            }
    ...
    def plot_training_progress(self):
        ...
        plot_path = os.path.join(self.plot_dir, 
                                f"training_progress_AFBCWNN_{self.system_order}D_"
                                f"mu{mu_str}_dt{self.config['base_interval']}_"
                                f"lambda{self.lambda_c}_beta{self.beta}"
                                f"{disturbance_str}.png")
    ...
    def replot_results(self, save_path=None):
        """Replot results without retraining"""
...
def plot_tracking_error_standalone(training_records, config=None, save_path=None):
    """Standalone tracking error plotting function with dynamic window boundaries"""
    ...
    # Dynamic window boundary computation
    if 'dwell_time' in df.columns:
    ...
    if save_path is None:
        ...
        if config:
            ...
            save_path = (f'tracking_error_AFBCWNN_{system_order}D_'
...
if __name__ == "__main__":
    ...
    # Configuration for 4D system (fully compatible with original)
    config_four_dim = {
        'j_init': 1,
        'mu': 1/3,
        'tran_r': 1,
        ...
        # System parameters (auto-compute control coefficients)
        'system_order': 4,  # 4th-order system
        'lambda_c': 2.0,    # λ=2, will auto-compute [8,12,6,1]
        'beta': 10.0,
    ...
    print("\n" + "="*50)
    print("Starting AFBCWNN Training")
    ...
    # Save Excel with auto-generated filename
    excel_filename = model.save_to_excel()
    
    # Plot training progress with auto-generated filename
    progress_plot = model.plot_training_progress()
    
    # Standalone tracking error plot with auto-generated filename
