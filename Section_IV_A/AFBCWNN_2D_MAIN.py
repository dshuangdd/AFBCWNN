"""
Adaptive Frequency-Based Constructive Wavelet Neural Network (AFBCWNN)

Standard version with cache size limit and memory monitoring (RSS tracking).

Key Features:
- Automatic trajectory processing with derivative cache
- Adaptive layer expansion with energy-based selection
- Pruning and recovery mechanisms
...
class AutomaticTrajectoryProcessor:
    ...
    def __init__(self, trajectory_func: Callable, system_order: int, device: str):
        ...
        self.max_cache_size = 10  # Cache only recent 10 time points
    ...
    def compute_derivatives(self, t: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # Limit cache size using FIFO strategy
...
class HybridWaveletControl(nn.Module):
    def __init__(self, config, device):
        ...
        # Compute control coefficients automatically
    ...
    def _compute_control_coefficients(self):
        """Compute control coefficients based on formula"""
        d = self.system_order
        λ = self.lambda_c
    ...
    def _compute_wavelet_terms_fast(self, x_est):
        """Fast computation of wavelet function values for all layers (memory optimized)"""
    ...
    def _wavelet_function_batch(self, x, centers, j, is_base_layer):
        """Batch computation of wavelet function (memory optimized)"""
        scale_factor = self.power_cache.get(j, self.inv_scaling ** j)
        
        # For very large batches, compute in chunks to reduce memory peak
        n_centers = len(centers)
        max_batch_size = 5000
        ...
        if n_centers <= max_batch_size:
            ...
            # Only chunk when center count is very large
            if is_base_layer:
                phi_list = []
                psi_list = []
    ...
    def energy_separate(self, layer_idx, stage):
        """Energy-based separation for layer expansion"""
        if layer_idx == 0:
    ...
    def _expand_layers(self):
        """Expand layers by adding new bases"""
    ...
    def _add_new_layer(self, parent_layer):
        ...
        # Key: when mu=1, new layer's active_stage should be set to max_stage-1 (fully activated)
        if self.mu == 1:
            self.active_stages.append(self.max_stage - 1)
        else:
    ...
    def _check_dwell_time(self, t):
        ...
        if current_time >= self.current_dwell_start + self.dwell_time - 1e-6:
            if self.dwell_losses:
                ...
                print(f"  DeltaT={self.dwell_time:.2f}, max_error={max_error:.6f}, avg_error={avg_error:.6f}")
                ...
                if hasattr(self, 'pending_pruning') and self.pending_pruning and self.stage == 1:
                    ...
                    if max_error <= self.delta_ac_prime:
                        self.N_counter += 1
                        self.dwell_time *= 2
                        print(f"  Error satisfied! N_counter={self.N_counter}/{self.N_d}, DeltaT->{self.dwell_time:.2f}")
    ...
    def _collect_weight_history(self):
        """Collect weight history for variance calculation"""
    ...
    def _execute_pruning(self):
        """Execute pruning and freezing mechanism"""
        ...
        print(f"  Frozen {frozen_count} low-variance weights (var < {self.xi_h})")
        
        if variance_info:
            print(f"  Variance stats: min={min(variance_info):.6f}, max={max(variance_info):.6f}, "
                  f"mean={np.mean(variance_info):.6f}")
            if n_samples_for_variance:
                print(f"  Based on last {n_samples_for_variance} samples (last dwell time: DeltaT={self.dwell_time/2:.2f})")
            else:
                print(f"  Based on all available historical samples")
    ...
    def _execute_recovery(self):
        ...
        print("\n[Stage 3] Executing recovery...")
        
        if self.backup_weights is None:
            print("  No backup weights available")
            return
        ...
        for layer_idx in range(len(self.weights)):
            if layer_idx in self.backup_weights:
                removed_mask = self.removed_indices[layer_idx]
                if removed_mask.any():
                    self.weights[layer_idx].data[removed_mask] = self.backup_weights[layer_idx][removed_mask]
                    self.removed_indices[layer_idx][removed_mask] = False
                    recovered_count += removed_mask.sum().item()
            
            frozen_mask = self.frozen_mask[layer_idx]
            if frozen_mask.any():
                self.frozen_mask[layer_idx][:] = False
                unfrozen_count += frozen_mask.sum().item()
        
        print(f"  Recovered {recovered_count} weights")
    ...
    def _update_all_parameters(self, wavelet_cache, deta):
        """Batch update all parameters (no cache version)"""
        self.adam_t += 1
    ...
    """
    ALTERNATIVE ADAM UPDATE: In-place operations version
    
    This version uses in-place operations for faster computation but may have lower precision.
    You can uncomment this version and comment out the high-precision version below if speed is critical.
    ...
    def _periodic_logging(self, t, deta):
        ...
        if current_time - self.last_print_time >= 0.05:
            # Parameter count excluding removed and frozen
            param_count = 0
            ...
            print(f"[t={current_time:.2f}] Stage={self.stage}, loss={deta.item():.6f}, "
                ...
                f"N_counter={self.N_counter}/{self.N_d}, DeltaT={self.dwell_time:.2f}, "
                f"RSS={current_rss:.2f}GB (+{rss_increase:.2f}GB)")
            
            # Detailed report every 10 time units
    ...
    def calculate_window_statistics(self, df):
        ...
        Prioritize using complete dwell_statistics, otherwise use training_records
        """
        window_stats = []
        
        # If complete dwell statistics exist, use them first
        if hasattr(self, 'dwell_statistics') and self.dwell_statistics:
            for i, dwell_stat in enumerate(self.dwell_statistics):
                ...
                stats = {
                    ...
                    'Mean |delta|': dwell_stat['avg_error'],
                    'Max |delta|': dwell_stat['max_error'],
                    'Min |delta|': dwell_stat['min_error'],
                ...
                if len(window_data) > 0:
                    stats['Max delta (with sign)'] = window_data['loss'].max()
                    stats['Min delta (with sign)'] = window_data['loss'].min()
            ...
            # Fallback: use original logic if dwell_statistics unavailable
                ...
                while i < len(df):
                    ...
                    if len(window_data) > 0:
                        stats = {
                            ...
                            'Mean |delta|': window_data['loss'].abs().mean(),
                            'Max |delta|': window_data['loss'].abs().max(),
                            'Min |delta|': window_data['loss'].abs().min(),
                            'Max delta (with sign)': window_data['loss'].max(),
                            'Min delta (with sign)': window_data['loss'].min(),
    ...
    def save_to_excel(self, filename=None):
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            if window_stats:
                ...
                max_errors = [w['Max |delta|'] for w in window_stats]
                print(f"  - Window max errors: min={min(max_errors):.6f}, max={max(max_errors):.6f}")
                
                # Find best window
                mean_errors = [w['Mean |delta|'] for w in window_stats]
                best_window = mean_errors.index(min(mean_errors)) + 1
                print(f"  - Best window: Window {best_window} (Mean |delta|={min(mean_errors):.6f})")
    ...
    def plot_training_progress(self):
        ...
        axes[1, 1].set_ylabel('Dwell Time (DeltaT)')
    ...
    def replot_results(self, save_path=None):
        """Replot results without rerunning training"""
...
if __name__ == "__main__":
    ...
    # Original 4D system configuration (fully compatible)
    config_four_dim = {
        'j_init': 1,
        'mu': 1/3,
        'tran_r': 1,
        ...
        # You can adjust the (lambda_c, beta) combination for different performance
        # Recommended combinations: (10,10), (20,10), (10,20), (20,20)
        'system_order': 4,
        'lambda_c': 2.0,    # lambda=2 will auto-compute [8,12,6,1]
        'beta': 10.0,
        
        # If not specified, use defaults (original system functions)
    ...
    config_two_dim = {
        ...
        # Recommended (lambda_c, beta) combinations: (10,10), (20,10), (10,20), (20,20)
        'system_order': 2,
        'lambda_c': 20.0,
        'beta': 20.0,
    ...
    # config = config_four_dim  # Use original system
    ...
    # Save Excel (filename auto-includes system parameters)
    excel_filename = model.save_to_excel()
    
    # Plot training progress (filename auto-includes system parameters)
    progress_plot = model.plot_training_progress()
    
    # Plot tracking error separately (filename auto-includes system parameters)
